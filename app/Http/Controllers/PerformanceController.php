...
class PerformanceController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('can:view performance')->only(['index']);
    }
    ...
    public function index()
    {
        ...
        // =======================================================
        // == DATA APLIKASI LARAVEL INI
        // =======================================================
        // Aktifkan query log sebelum query apapun
        DB::enableQueryLog();
        // Lakukan query dummy agar query log tidak kosong
        DB::table('users')->get();

        $appData = [
            'memoryUsage' => $this->getAppMemoryUsage(), // Penggunaan RAM oleh skrip ini
            'executionTime' => $this->getRequestExecutionTime(), // Waktu proses request
            'cpuUsage' => $this->getAppCpuUsage(), // Waktu CPU yang dipakai skrip ini
            'queryCount' => count(DB::getQueryLog()), // Jumlah query DB untuk request ini
            'queries' => DB::getQueryLog(), // Detail query
            'activeUsers' => DB::table('sessions')->count(),
            'jobsPending' => DB::table('jobs')->count(),
        ];

        return view('performance.index', compact('globalData', 'appData'));
    }

    // =======================================================
    // == FUNGSI-FUNGSI BARU UNTUK APLIKASI
    // =======================================================

    /**
     * Mengukur penggunaan memori puncak (peak usage) oleh aplikasi untuk request ini.
     */
    private function getAppMemoryUsage()
    {
        // memory_get_peak_usage() memberikan byte, kita ubah ke MB
        $memoryUsageBytes = memory_get_peak_usage(true);
        return round($memoryUsageBytes / 1024 / 1024, 2); // dalam MB
    }

    /**
     * Mengukur total waktu eksekusi request dari awal sampai titik ini.
     */
    private function getRequestExecutionTime()
    {
        // LARAVEL_START didefinisikan di public/index.php
        $startTime = defined('LARAVEL_START') ? LARAVEL_START : microtime(true);
        $executionTime = (microtime(true) - $startTime) * 1000; // dalam milidetik (ms)
        return round($executionTime);
    }

    /**
     * Mengukur waktu CPU yang dihabiskan oleh aplikasi untuk request ini.
     */
    private function getAppCpuUsage()
    {
        // getrusage() mengembalikan waktu dalam microsecond
        $usage = getrusage();
        $userCpuTime = $usage['ru_utime.tv_sec'] * 1e6 + $usage['ru_utime.tv_usec'];
        $systemCpuTime = $usage['ru_stime.tv_sec'] * 1e6 + $usage['ru_stime.tv_usec'];

        // Total waktu CPU dalam milidetik (ms)
        return round(($userCpuTime + $systemCpuTime) / 1000);
    }

    // =======================================================
    // == FUNGSI-FUNGSI LAMA (UNTUK SERVER) - Rename agar lebih jelas
    // =======================================================
