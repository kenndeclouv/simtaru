                    ...
                    <tbody>
                        @foreach ($logs as $log)
                            <tr>
                                <td>{{ $log['name'] }}</td>
                                <td>{{ $log['size'] }}</td>
                                <td>{{ date('d F Y H:i:s', $log['modified']) }}</td>
                                <td>
                                    <a href="{{ route('logs.show', ['filename' => $log['name']]) }}"
                                        class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="top"
                                        data-bs-title="Lihat Log">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <x-delete :route="route('logs.destroy', $log['name'])" title="Hapus Log" />
                                    <a href="{{ route('logs.download', ['filename' => $log['name']]) }}"
