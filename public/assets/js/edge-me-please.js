  ...
  var start = function (
    ...
    var showProgressAndGoOn = function (timeout, callback, bar) {
      ...
      var continueProgress = function () {
        ...
        if (percent > 100) {
          ...
          setTimeout(continueProgress, 100);
        }
      };
      continueProgress();
    };
    ...
    var goOn = function () {
      ...
      if (duration < targetDuration || passType == "go") {
        ...
        showImage(passType);
        var randomMessage =
          ourMessages[Math.floor(Math.random() * ourMessages.length)];
        $("#message").html(randomMessage[0]);
        updateFlash(randomMessage[2]);
        showProgressAndGoOn(
          randomMessage[1] * 1000 * multiplier,
          goOn,
          "jerkbar"
        );
      } else {
